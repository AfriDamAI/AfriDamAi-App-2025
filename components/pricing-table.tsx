...
const PLANS = [
  ...
  },
  {
    ...
    buttonText: "Try for $3 (One-Time)",
    highlight: false,
    pro: "Skincare Consultant / Dermatologist",
    color: "#E1784F",
    billing: " once" // One-time label
  },
  {
    name: "Standard",
  ...
  return (
    <div className="w-full space-y-12">
      ...
      <motion.div 
        ...
        className="relative group overflow-hidden bg-[#1C1A19] dark:bg-[#1C1A19] border border-white/5 rounded-[2.5rem] p-8 md:p-10 shadow-2xl"
      >
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
          <div className="flex items-center gap-6 text-center md:text-left flex-col md:flex-row">
            <div className="w-14 h-14 bg-[#E1784F] rounded-2xl flex items-center justify-center shadow-lg shrink-0 animate-pulse">
            ...
            <div className="space-y-1">
              ...
              <p className="text-[#4DB6AC] font-black text-[9px] uppercase tracking-[0.3em] flex items-center justify-center md:justify-start gap-2">
                <Clock size={12} className="animate-spin-slow" /> Instant Connection • Skip the wait
              </p>
            </div>
          </div>
          <div className="flex items-center gap-8 flex-col md:flex-row">
            ...
            <button className="px-10 py-5 bg-white text-[#1C1A19] font-black rounded-2xl hover:bg-[#E1784F] hover:text-white transition-all shadow-xl uppercase tracking-widest text-[10px] flex items-center gap-3">
        ...
        {PLANS.map((plan, i) => (
          <div 
            ...
            className={`relative p-8 rounded-[3rem] border transition-all flex flex-col justify-between group overflow-hidden ${
              plan.highlight 
                ? 'bg-white dark:bg-[#1C1A19] border-[#E1784F] shadow-2xl scale-[1.02] z-10' 
                : 'bg-white/50 dark:bg-white/5 border-border hover:border-[#E1784F]/30 shadow-sm'
            }`}
          >
            ...
            {plan.highlight && (
              <div className="absolute top-6 right-8">
                <div className="w-8 h-8 bg-[#E1784F]/10 rounded-full flex items-center justify-center">
                    <Heart className="w-4 h-4 text-[#E1784F] fill-[#E1784F]" />
                </div>
              </div>
            )}
            ...
            <button className={`w-full mt-10 py-5 rounded-2xl font-black uppercase tracking-widest text-[9px] transition-all ${
              plan.highlight 
                ? 'bg-[#E1784F] text-white hover:brightness-110 shadow-lg shadow-[#E1784F]/20' 
                : 'bg-muted text-foreground hover:bg-[#1C1A19] hover:text-white dark:hover:bg-white dark:hover:text-black'
            }`}>
              {plan.buttonText}
            </button>
          </div>
