...
function mapProducts(data: any[]): Product[] {
  return data.map((p: any) => ({
    ...
    skinType: p.skinType || p.skin_type || "",
    rating: p.rating || 5,
    inStock: p.stock > 0,
  }));
}
...
export default function MarketplacePage() {
    ...
    (base: Product[]) => {
      ...
      if (selectedSkinType !== "All") {
        filtered = filtered.filter((p) =>
          p.skinType?.toLowerCase().includes(selectedSkinType.toLowerCase())
        );
      }
  ...
  }, [searchQuery, selectedCategory, selectedSkinType, maxPrice, allProducts, applyClientFilters]);
    ...
    <main className="min-h-[100svh] bg-white dark:bg-[#050505] text-black dark:text-white transition-colors duration-500 pb-12 selection:bg-[#E1784F]/30 text-left">
      ...
      <div className="max-w-screen-xl mx-auto px-4 py-6 lg:py-10 relative z-10 space-y-8">
        ...
        <header className="space-y-6">
            ...
            {showFilters && (
              ...
              >
                <div className="space-y-5 pt-2 pb-5 border-b border-gray-100 dark:border-white/10">
                  ...
                  <div className="space-y-2">
                    <p className="text-[8px] font-black tracking-[0.3em] opacity-40">SKIN TYPE</p>
                    <div className="flex flex-wrap gap-2">
                      {SKIN_TYPES.map((type) => (
                        <button
                          key={type}
                  ...
                  <div className="space-y-2">
                    <p className="text-[8px] font-black tracking-[0.3em] opacity-40">MAX PRICE (₦)</p>
                    <input
                      type="number"
                      value={maxPrice}
                      onChange={(e) =>
                      ...
                      min={0}
                      className="bg-gray-50 dark:bg-white/5 border border-gray-100 dark:border-white/10 rounded-xl py-2 px-4 text-[10px] font-bold tracking-widest focus:border-[#E1784F] outline-none transition-all w-40"
                    />
                  </div>
          ...
          {!showFilters && (selectedCategory || selectedSkinType !== "All" || maxPrice !== "") && (
            <div className="flex flex-wrap gap-2">
              {selectedCategory && (
              ...
              {selectedSkinType !== "All" && (
                <span className="flex items-center gap-1 px-2.5 py-1 bg-[#E1784F]/10 text-[#E1784F] rounded-lg text-[8px] font-black tracking-wider">
                  {selectedSkinType}
                  <button onClick={() => setSelectedSkinType("All")} className="ml-0.5">
                    <X size={8} />
                  </button>
                </span>
              )}
